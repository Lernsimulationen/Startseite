...
const DEBATE_FINALE={
  ...
  ownPositionPrompt:"Formuliert eine dritte, differenzierte Position, die Stärken beider Seiten aufnimmt.",
  arguments:[
...
const ETHIK_KOMPASS={
  ...
  intro:"Diese Leitfrage begleitet den ganzen Kurs. Die Bergpredigt-Expedition untersucht vier Antworten Jesu darauf - als vier Kompass-Nadeln, die euren Lehrkreis am Hang in vier Stationen gliedern. Am Ende führt die Klasse alle vier Nadeln in einer eigenen \"Bergpredigt für heute\" zusammen.",
  needles:{
...
function activePrompt(rows){const internal=["feedback","product","miss","score","card","debate","focus"].map(prefix=>`${prefix}:`);return controlRows(rows).filter(row=>row.event_type==="prompt"&&row.payload&&!internal.some(prefix=>String(row.payload).startsWith(prefix))).at(-1)?.payload||""}
...
function taskSkillLabel(task){return{cards:"Karten deuten",match:"Zuordnen",quest:"Moralquest",puzzle:"Puzzle",logic:"Logikrätsel",levels:"Levelspiel",unlockword:"Buchstaben-Rätsel",chat:"Chat-Simulation",wimmel:"Wimmelbild",path:"Lernpfad",finalquest:"Finalquest"}[task?.type]||"Spiel bearbeiten"}
...
function groupGameCounts(rows){const counts=new Map();cardRows(rows).forEach(card=>counts.set(card.group,(counts.get(card.group)||0)+1));return counts}
